<?php
    ...
    // $link = new mysqli("localhost", "root", "", "reviews");
    ...
    // $link->query("ALTER TABLE reviews ADD COLUMN company VARCHAR (20) AFTER id;");
    // $link->query("ALTER TABLE reviews ADD COLUMN date VARCHAR (50) AFTER company;");
    ...
    function mostrar_array($datos, $link) {
        foreach($datos as $dato) {
            $Query = "INSERT INTO reviews (name, city, project, rdate, review, start, id, company, date) 
                      VALUES ('".$dato['Review']."','-','".$dato['Project']."','".date('Y-m-d')."','".$dato['Comment']."','".$dato['stars']."','', '".$dato['company']."','".$dato['date']."');";
            if ($link->query($Query)){
                echo "{$dato['date']} -> {$dato['Review']}<br/>";
            }
        }
    }

    // usort($Testimonials, 'ordenar');
    // mostrar_array($Testimonials, $link);
?>